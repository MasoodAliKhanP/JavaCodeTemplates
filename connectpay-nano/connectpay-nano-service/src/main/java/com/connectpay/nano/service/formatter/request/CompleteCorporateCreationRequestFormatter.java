...
@Component
public class CompleteCorporateCreationRequestFormatter implements RequestDataFormatter {
	@Value("${nano.url.complete}")
	private String nanoCompleteCorporateUrl;
	...
	public <T> IntegrationPropertiesBean fetchRequestBody(T tTypeObject) {
		...
		String url = nanoCompleteCorporateUrl.replace("{id}", partyId);
